  ...
  return (
    <section id="pricing" className="py-8 px-4 sm:px-6 max-w-4xl mx-auto border-t border-black/[0.06]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* FREE */}
        <div className="rounded-2xl border border-black/[0.08] bg-white shadow-sm p-6 flex flex-col gap-4">
          <div>
            <div className="text-slate-400 text-xs font-black uppercase tracking-widest mb-1">{free.name}</div>
            <div className="text-3xl font-black text-slate-900">{free.price}</div>
            <div className="text-slate-400 text-xs mt-0.5">{free.period}</div>
          </div>
          <ul className="flex flex-col gap-2.5 flex-1">
            {free.features.map(f => (
              <li key={f.text} className="flex items-center gap-2.5 text-sm">
                <span className={f.included ? 'text-green-600' : 'text-slate-300'}>{f.included ? '✓' : '✗'}</span>
          ...
          <Link
            href={free.cta.href}
            className="mt-2 block text-center px-5 py-3 rounded-xl border border-black/[0.12] text-slate-600 text-sm font-bold hover:border-black/25 hover:text-slate-900 transition-all"
          >
            {free.cta.text}
          </Link>
        </div>
        ...
        {/* PRO */}
        <div
          className="rounded-2xl border p-6 flex flex-col gap-4 relative overflow-hidden bg-white"
          style={{ borderColor: 'rgba(2,132,199,0.35)', boxShadow: '0 0 32px rgba(2,132,199,0.08)' }}
        >
          {pro.badge && (
          ...
          <div>
            ...
            <div className="text-3xl font-black text-slate-900">{pro.price}</div>
            <div className="text-slate-400 text-xs mt-0.5">{pro.period}</div>
          </div>
          <ul className="flex flex-col gap-2.5 flex-1">
            {pro.features.map(f => (
              <li key={f.text} className="flex items-center gap-2.5 text-sm">
